...
func createErrorsGo() {
	...
	for _, resource := range resources {
		switch resource {
		case "OK":
			buf.WriteString("OK = 0\n\n")
		case "USAGE":
			buf.WriteString("USAGE = iota + 63\n")
		default:
			fmt.Fprintf(&buf, "%s\n", resource)
		}
	}
...
`, top, name, top, top, name, top)
		fmt.Fprintf(&buf, "// Make%s will return %s struct as error\n", funcName[resource], name)
		fmt.Fprintf(&buf, "func Make%s(err error) error { return %s{base{err: err, code: %s}} }\n",
			funcName[resource], name, resource)
	}
	...
	fh.Write(formatted)
	fmt.Println("errors.go is completed")
}

func createErrorsTestGo() {
	var buf bytes.Buffer
	buf.WriteString("package exit\n\n")
	buf.WriteString("// Auto generated by `mkerrs` in internal/cmd/mkerrs/main.go\n")
	buf.WriteString("// Do not edit this.\n")
...
`, funcName[resource], name, resource, resource)
		buf.WriteString("}\n\n")
	}
	...
	fh, err := os.Create("errors_test.go")
	if err != nil {
		log.Fatalf("%s", err.Error())
	}
	defer fh.Close()
	...
	fh.Write(formatted)
	fmt.Println("errors_test.go is completed")
}
